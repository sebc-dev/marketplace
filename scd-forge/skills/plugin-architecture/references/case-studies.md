...
Unlike scd-astro, article-writing keeps all knowledge in the skill bodies themselves. Each detector skill is essentially a reference on its topic.
...
2. **Action commands** (scd-astro): `/scd-astro:audit`, `/scd-astro:scaffold`. Independent, each does one thing.
3. **Workshop commands** (scd-forge): `/scd-forge:design`. Interactive, multi-step conversation.
...
Use `/scd-forge:distill` for guided assistance with this migration.
</migration_patterns>
