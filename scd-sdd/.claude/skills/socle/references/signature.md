...
`quality-config-guard` ne convient pas ici : `references/guards.md`, § `verifier-guard`. C'est cette
...
par `/scd-sdd:guards`, à son **étape 7** et alors seulement, quand le garde `verifier-guard` est
retenu. Un projet qui ne pose pas le garde ne la lit jamais — c'est le motif pour lequel elle vit
dans un fichier séparé (`DECISIONS.md` §D20, §D26).

⚠️ `references/guards.md` est **toujours déjà chargée** quand celle-ci l'est — l'étape 1 de la
commande précède l'étape 7. C'est ce qui autorise les renvois vers elle ci-dessous : ils résolvent
dans la fenêtre, ils ne renvoient pas vers un fichier absent.

⚠️ **`/scd-sdd:signer` ne la charge PAS**, et c'est délibéré (§D40, écarté n° 3) : la connaissance
dont cette commande a besoin n'est pas ici mais dans le `docs/ci.md` **du projet**. Charger ces
lignes pour préparer un commit serait exactement le coût que §D35 corrige.
...
C'est une dépendance de moins — et la règle de maturité de `references/guards.md` s'applique d'autant
...
MOTIFS='...'                     # les motifs de la table de references/guards.md, écosystème du projet
...
Les deux constats sont dans `references/guards.md` — § `verifier-guard` pour le vert local qui signifie
...
`@ts-ignore`, `as any`, `nosemgrep` apparaissent **ici aussi**, en plus de `references/guards.md` et du
`docs/ci.md` produit. C'est un fichier de plus sur lequel un garde trop large se bloquerait : le
motif est en § `verifier-guard` de `references/guards.md`. Ce qui en découle pour l'esquisse ci-dessus :
