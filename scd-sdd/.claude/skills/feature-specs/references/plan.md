...
(`docs/technique.md`, `docs/adr/`) — il ne re-décide pas ce qui est déjà tranché. Produit **en plan
...
gamme se renouvelle, et un nom de modèle écrit ici périmerait en silence.)* Trace vers spec +
technique + ADR. Concis : un plan court a un meilleur taux d'acceptation qu'un plan fleuve.
</role>

<template>
```markdown
# Plan technique : [feature]
Trace vers : specs/NNN-feature/spec.md · docs/technique.md · docs/adr/

## Approche
[2-4 phrases : la stratégie retenue.]

## Réutilisation du socle
- Stack imposée par docs/technique.md : [langage/framework/DB/… déjà décidés — ne pas re-choisir]
- Invariants d'architecture (docs/technique.md) qui s'appliquent : [I1, I3 — ou « aucun »]
...
<guidance>
- **Ne re-décide rien du socle.** Langage, framework, DB, auth, déploiement sont fixés par `docs/technique.md` et `docs/adr/`. Le plan les *applique*. Contredire un ADR accepté = interdit (le hook `block-adr-edits` empêche d'ailleurs de le réécrire).
...
- **Les invariants de `docs/technique.md` se confrontent, ils ne se re-décident pas.** Un lot qui franchit une frontière, inverse un sens de dépendance ou place un artefact hors du dossier prescrit change de découpage — ou **écrit sa dérogation avec sa raison**, en nommant l'invariant. Muette, elle est un **Major** à la gate `analyze` ; répétée d'une feature à l'autre, elle dit que l'invariant est périmé et relève de `/scd-sdd:technique`, jamais d'une édition d'ici. Table d'invariants absente ou vide → la ligne n'existe pas et l'étape est annoncée sautée.
...
- [ ] Si `docs/technique.md` porte une table d'invariants : les fichiers touchés y ont été
      **confrontés**, et toute frontière franchie est **nommée et justifiée**. Table absente ou
      vide, l'étape a été annoncée sautée.
