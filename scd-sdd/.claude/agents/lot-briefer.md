...
- **Ne devine pas le mode.** Lis `_vérif :_` tel qu'écrit ; absent → `TDD`. Ne « corrige » jamais un mode que tu jugerais mal choisi — c'est un finding amont (`scd-sdd:analyze`), pas ton rôle. Reporte seulement.
