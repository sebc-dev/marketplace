...
- Si la branche `reland/<slug>-<lot>` **existe déjà** (relance partielle) → `{ status: 'blocked-reland-exists', relandBranch: "reland/<slug>-<lot>", note: "branche reland préexistante — inspecte/supprime avant de relancer (l'idempotence est gérée en amont par /scd-sdd:reland)" }`. Ne l'écrase pas silencieusement.
...
- Un lot déjà arrivé dans `main` n'est pas orphelin : l'idempotence (ne pas relander deux fois) est assurée **en amont** par `/scd-sdd:reland` ; si tu détectes une branche `reland/…` préexistante, STOP `blocked-reland-exists`.
</constraints>
