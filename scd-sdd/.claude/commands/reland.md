...
Tu **rattrapes un lot orphelin**. Le stacking a une faille vécue : si un humain merge une PR empilée
alors que sa base est encore la branche de lot intermédiaire `impl/<slug>-Rk`, GitHub fusionne les
...
Tu **détectes** les lots orphelins, tu **calcules** de façon déterministe les commits propres à
rapatrier, puis tu **délègues** le rattrapage à l'agent `relander` (branche `reland/<slug>-Rn` depuis
...
- **Idempotent.** Relançable sans effet : un lot déjà rapatrié (dans `main`, ou avec une PR
  `reland/…` existante) est **sauté**, pas recréé.
- **Curatif ciblé.** Ne relande que des lots **`MERGED` hors de `défaut` et absents de `main`**. Un
  lot ouvert, ou déjà dans `main`, n'est pas concerné.
- Tu ne modifies ni `tasks.md`, ni le contrat, ni le code : tu rapatries des commits déjà produits.
...
2. **Prépare les signaux.**
   ...
   - Lis `origin/<défaut>:specs/<NNN-slug>/tasks.md` via `git show` — les cases **telles qu'elles sont
     dans `main`**, à ne pas confondre avec le `tasks.md` local, qui peut porter des cases cochées sur
     une branche de lot non mergée. C'est le **signal de contenu**.

3. **Détecte les lots orphelins.** Pour chaque lot `Rn` de `tasks.md` (ou le seul `Rn` fourni),
   récupère sa PR : `gh pr list --head impl/<slug>-Rn --state all --json
   number,state,baseRefName,headRefName,headRefOid` (équivalent `glab mr list`). Un lot est
   **🔴 ORPHELIN** ssi **toutes** ces conditions :
   1. sa PR est **`MERGED`** ;
   2. sa base au merge `baseRefName` **≠ `défaut`** (mergée dans une branche de lot `impl/<slug>-Rk`) ;
   3. le lot est **absent de `main`** : ses `Tn` ne sont **pas** cochés dans
      `origin/<défaut>:tasks.md` (signal de contenu, prioritaire), **corroboré** par
      `! git merge-base --is-ancestor <headRefOid> origin/<défaut>` (code ≠ 0).

   **Idempotence — no-op si déjà rapatrié.** Si le lot est **déjà arrivé dans `main`** (Tn cochés dans
   `origin/<défaut>`, ou `git merge-base --is-ancestor <headRefOid> origin/<défaut>` code 0) → **pas
   orphelin**, ne relande pas. Si une branche ou PR `reland/<slug>-Rn` **existe déjà**
   (`git ls-remote --heads origin reland/<slug>-Rn` non vide, ou
   `gh pr list --head reland/<slug>-Rn --state all`) → le rattrapage est déjà en cours ou fait :
   ...
   **Cas limite — base auto-retargetée.** Si `baseRefName` == `défaut` (GitHub a auto-retargeté la PR
   après suppression de la branche de lot au merge), le lot n'est **pas** orphelin au sens
   base ≠ défaut : fie-toi alors au seul signal contenu/ancêtre pour décider s'il est arrivé. **Ne
   signale pas d'orphelin à tort.**

   Aucun lot orphelin → annonce « aucun lot orphelin à rapatrier » et STOP.
...
4. **Calcule le range et délègue — dans l'ordre des dépendances.** Traite les orphelins dans l'ordre
   ...
   incomplète. Pour chaque orphelin `Rn` mergé dans `impl/<slug>-Rk` :
   ...
   - récupère `capability` (titre du lot) et les `shalls` (FR/SHALL livrés, depuis `spec.md`) pour la
     description.

   Invoque l'agent **`scd-sdd:relander`** (Task) avec
   `{ featureDir, slug, lot: "Rn", base: "<défaut>", oldBase: "impl/<slug>-Rk",
   headRefOid: "<oid>", orphanPr: <k>, capability, shalls }`. L'agent : arbre propre exigé → branche
   `reland/<slug>-Rn` depuis `origin/<défaut>` → cherry-pick des commits propres → push simple → PR
...
- Tu ne relandes pas un lot déjà dans `main`, ni un lot dont la PR est encore ouverte.
- Tu ne recrées pas une branche `reland/…` existante — tu la signales.
- Tu ne lances aucun lot (`/scd-sdd:run`), tu ne rebases aucune PR ouverte (`/scd-sdd:sync`).
...
- **Résultat** : l'action effectuée et les deux PR en jeu. Succès :
  `Rn rapatrié depuis PR #6 (orpheline) · nouvelle PR #21 ready → main`. Rien à faire :
  `Rn — aucun orphelin` ou `Rn déjà dans main — sauté`. Blocage :
  `⛔ blocked-conflict · PR #6 reste orpheline`.

Un rattrapage **et** un no-op idempotent se consignent tous deux : la ligne « déjà dans main —
sauté » est ce qui évite de rouvrir la question au prochain doute.

Un orphelin bloqué est le cas le plus important à tracer : son code est **absent de `main`** et rien
sur disque ne le dit.
...
Renvoie vers `/scd-sdd:status-impl NNN` pour reconfirmer que `main` a bien reçu le code — c'est la
