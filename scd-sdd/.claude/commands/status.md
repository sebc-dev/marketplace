---
description: "Vue d'ensemble des trois niveaux du cycle — socle, specs, implémentation — en une seule lecture. Dérive l'état des fichiers, croise docs/JOURNAL.md pour les faits non dérivables (verdict analyze, issue des lots) en contrôlant leur péremption, et donne UNE prochaine commande. Le point d'entrée quand on rouvre un projet. Lecture seule."
...
- **`docs/JOURNAL.md`**, qui donne ce que les fichiers ne portent pas : verdict d'une gate
  `analyze`, `premortem` appliqué, issue d'un lot (y compris un run **bloqué**, qui ne coche rien
  et n'ouvre aucune PR) ;
- **les dates**, qui décident si une ligne de journal vaut encore quelque chose.

Ratio : 5% humain / 95% AI (lecture mécanique ; l'humain choisit la suite).

## Règles absolues

- **Lecture seule.** Tu ne modifies aucun fichier — **pas même `docs/JOURNAL.md`**, et tu n'as
  pas `Edit`. Tu ne joues aucune phase, donc tu ne consignes rien : c'est de nature, pas un
  oubli.
...
- **Les runs en vol sont hors périmètre.** Tu ne scannes ni `git worktree list` ni les branches
  `impl/<slug>-Rn`. Un run interrompu se voit **au journal** (`⛔ blocked-*`), pas ailleurs.
...
5. **Lis `docs/JOURNAL.md`** s'il existe. Pour chaque feature, dans sa section `## NNN-slug`,
   relève la dernière ligne `analyze` (verdict + date), la dernière ligne `premortem`, et la
   dernière ligne `run Rn` de chaque lot. Dans `## Socle`, relève simplement les phases jouées.
...
7. **Compose les derniers événements** : les 3 à 5 dernières lignes du journal, **toutes sections
   confondues**, les plus récentes d'abord. C'est la chronologie, pas un état — ne la commente
   pas, elle se lit seule.

8. **Choisis LA prochaine commande**, une seule, par ordre de priorité :
...
9. **Produis le rapport** selon le bloc `<report>`, avec la prochaine commande de chaque feature
...
### Derniers événements — docs/JOURNAL.md
30/07  run R2    ✅ done · TDD · 4 tests · PR #12
28/07  analyze   PRÊT — 0 Critical
...
- **`docs/JOURNAL.md` absent** (projet démarré avant le journal, ou avec les trois anciens
  plugins) → **vue dérivée complète, sans la section « Derniers événements »**, et une ligne de
  pied qui le dit : « Pas de `docs/JOURNAL.md` — le verdict `analyze` et l'issue des runs ne sont
...
- **Section `## NNN-slug` absente pour une feature existante** → phase dérivée seule, sans date.
...
étape 8 — et dis en une ligne pourquoi c'est celle-là.
