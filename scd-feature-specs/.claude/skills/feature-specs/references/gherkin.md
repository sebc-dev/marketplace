...
- **Qui l'exécute** : pas nous. Le `.feature` fait partie du **contrat** remis à l'aval — ce plugin n'exécute aucun test. `analyze` contrôle seulement qu'il est bien **dérivé d'un `SHALL`** et bien formé ; c'est le workflow d'implémentation qui le fera passer au vert.
</guidance>
