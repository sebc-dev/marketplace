...
description: |
  ...
  listing — read this first: the rules that silently degrade French, the licence each one really
...
marketplaces is *a Markdown file and a week-old account* — no signing, no review, no sandbox.
...
**`humanizer` (`blader/humanizer`, MIT, v2.9.1, 412 lines, 33 numbered patterns).** Three of its
rules are hostile to French:

| Pattern | What it does | In French |
|---|---|---|
| **§14 Em dashes (and en dashes): cut them** | *"The em dash is one of the most reliable AI tells, so treat this as a hard constraint"* — scans the final rewrite for `—` and `–`, any hit means not done | **False friend.** The em dash is standard French punctuation for dialogue, incise and enumeration (OQLF). Banning it damages correct French |
...
**§14 has an escape hatch and the other two do not.** The file declares it at the top: *"A sample
outranks this skill's style rules, including the em dash rule in §14: if the sample uses em dashes,
keep them at roughly the sample's frequency."* So supplying a French writing sample neutralises §14
by design. §17 and §19 have no equivalent — they must be removed, or the skill's output must be
re-checked against French typography afterwards.
...
writes `Do not keyword-stuff — write for humans first`, with an em dash, in the very file that
`humanizer` would flag as one of the most reliable AI tells. Neither of them settles it. In French it
is settled by typography, not by either skill: the dash stays.
...
| `doc-coauthoring` | **none at all** — no own `LICENSE.txt`, no repository licence. `THIRD_PARTY_NOTICES.md` covers embedded dependencies only, and the README says *"**Many** skills in this repo are open source (Apache 2.0)"* — a statement, not a grant, and "many" is not "all" |
...
| `brand-voice` | MIT, **© Tribe AI** — hosted by Anthropic, owned by a partner. "Official" here means hosted, not authored |
...
- **`brand-voice` is not a skill, it is a full plugin** — agents, commands, settings, and **three**
...
technology's own vendor could be embedded; a third-party server is documented — install command
...
gaps this plugin fills — the case for building rather than adopting is now about governance, not
about absence.
