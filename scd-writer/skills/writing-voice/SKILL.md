...
name: writing-voice
description: |
  The author's editorial identity: six voice rules, the banned dash, the anti-fabrication rule,
  and the draft-time filter of banned vocabulary and rhetorical patterns that belongs in context
  while writing. Reach it whenever the author's text is written, reviewed, polished or translated.
...
The voice is the six rules below, the banned dash, the banned vocabulary, the forbidden patterns, and the anti-fabrication rule. It is identical in a blog article, a LinkedIn article, and a LinkedIn post. It does not soften for an employer or sharpen for a prospect.
...
## Banned punctuation: the dash

**No `—` and no `–` between clauses. In French, in English, in every phase, without exception.**

This is a voice rule, not a typographic claim, and the distinction matters because the two point in opposite directions. French typography is on the other side: the OQLF records the cadratin as correct punctuation for dialogue, incise and enumeration, and nothing here contradicts it. The rule stands on how the character is *read* rather than on whether it is correct. In 2026 it signs "written by a model" to the audience this author writes for, and a correct mark that costs the reader's trust is not worth what it buys.

Because it is a voice rule and not a detector threshold, **it sits above the arbitration layer**. It is not softened by:

- the author's own sample (`faux-positifs` precedence rule 1). The author has ruled, and the ruling is this one;
- French typography, or a source that vindicates the character;
- the length of the piece, its type, its channel, or the genre matrix;
- a parenthetical that would genuinely read better with a dash. That case is real and it still loses.

**What replaces it**, chosen per occurrence and never mechanically:

| The dash was doing | Replace with |
|---|---|
| An incise inside a sentence | Commas, or parentheses when the aside is a true digression |
| Announcing an explanation, an example or a list | A colon |
| Joining two independent statements | A full stop. Two sentences |
| Trailing an afterthought at the end | A full stop, or nothing: this is usually the decorative case, and the clause goes with the dash |

**The substitute is banned with it.** A spaced hyphen ` - ` between clauses is the same tell in a cheaper costume, and so is a semicolon used where the dash used to sit. A draft whose dashes all came back as spaced hyphens was not fixed, it was re-dressed.

**One exception, and it is not punctuation.** `–` stays inside a numeric or reference range (`800–2 000 mots`, `2024–2025`, `R1–R7`), where it separates nothing and carries no rhetoric.

This is the one place the rule is stated. `structure-symetrique` #13, `fausse-profondeur` #2 and `faux-positifs` apply it; they do not redefine it, and none of them holds a threshold for it any more.
...
**Protects.** The reader's first thirty seconds, which on a feed are the only ones you get.
...
The exhaustive catalogs live in the detectors, which load at `/review` and `/polish`. What follows is the short list, and it stays always-active for one reason: it has to be in context while the draft is being *written*. A marker not written costs nothing; the same marker caught later costs a round trip. The dash rule above belongs to this moment too, and it is the cheapest of all of them to honour: it costs one keystroke not taken.
...
- Preserve the author's imperfections, which are often what makes the writing human
...
**The dash is the one exception to this principle**, and it is deliberate. It is not a marker to weigh, it is a character the author has ruled out, so `/polish` replaces it and shows the replacement as a tagged modification the author accepts or refuses. Everything else on this page is still reported, never fixed.
