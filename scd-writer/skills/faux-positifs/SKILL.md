...
description: |
  ...
  applied to French: straight quotes, Title Case, a translated blocklist. Also carries the one
  signal it may never kill, the banned dash.
---
...
## The one signal this page may not kill

**`—` and `–` between clauses are banned by `writing-voice`, and nothing on this page reaches them.** Not precedence rule 1, not the author's own sample, not French typography, not a channel, not a genre. A dash is never a false positive, never reported with doubt, and never "settled" by an earlier defence.

It is written here rather than only in `writing-voice` because this is the page that would otherwise cancel it: every mechanism below is designed to protect legitimate French punctuation from English heuristics, and the dash is exactly the shape of thing those mechanisms save. The author has ruled the other way. Read the rule and its replacement table in **writing-voice**; apply it, do not re-litigate it.

The rest of this page is unchanged and applies to everything else.
...
1. **The author's own sample outranks the generic rule.** If the author has supplied a sample of their published writing and it does the flagged thing habitually, the flag is a false positive. The catalogs describe LLM writing statistically; the sample describes *this* author. Say so and move on, and do not flag the same habit on every paragraph.
...
Rule 3 has one edge worth naming, because it is the only way a dash legitimately survives: a dash *inside a quotation* stays, like everything else the author did not write. It is not the author's punctuation, and correcting it would misquote the source.
...
| Parenthesis or colon | The author's habitual punctuation, and the two marks the dash rule sends work to | Check the sample. Density rises once dashes are gone, and that rise is the rule working, not a signal |
...
**The dash is absent from this table, and the absence is deliberate.** By the test above it would qualify: banning `—` *is* an English heuristic fired on French, and the OQLF is on its side. It is out because `writing-voice` bans the character as a rule of the author's voice, which outranks typography and outranks this page. Do not restore the row on the strength of the source. The source was never in dispute, and it does not decide this.

This section applies to third-party skills as much as to this plugin's own detectors. `humanizer`, the most widely installed de-AI-ing skill, hard-codes three of these rows; `skills-tiers` carries which of its rules have an escape hatch, which must be removed outright, and which one now agrees with this plugin.

**Reporting.** A finding killed by this section is worth one line, once per piece, not per
occurrence: *"four straightened quote pairs restored to « … », French typography, not a marker."*

## Arbitration between detectors

This table is where detector overlaps are settled, and it is the one place. When several detectors fire on the same span, report it **once**, under the detector that owns the deepest cause:

| Overlap | Reported under | Because |
|---------|---------------|---------|
| Dash / anything else on the same span | The dash, always | It is not a weighed signal, it is a banned character. Report it, then report the other finding separately if it survives |
...
Three properties: the line, the signal and its detector, and the reason for doubt. Never more than three lines. If a family recurs, report it once with a count ("four straightened quote pairs, French typography, not flagged individually") rather than four times.

A dash never takes this format. It has no doubt to show, so it is reported flatly, counted, and listed by line.
...
**No false-positive rate has been measured for non-native French writers.** The one solid figure in the literature, 61.22% of TOEFL essays misclassified as AI-generated and dropping to 11.77% after lexical enrichment (Liang et al., *Patterns* 4(7), 2023), is English. The mechanism behind it (low perplexity, low lexical diversity) is exactly what `marqueurs-lexicaux` measures, so the risk plausibly transfers to French, but no study has measured it. Consequence: when the author is not a native French speaker, or is writing in a second register, **precedence rule 1 (the author's own sample) is not a convenience, it is the only available correction.** Ask for a sample rather than trusting the statistical signal, and say that the risk is unquantified in French rather than putting a number on it.

Neither gap touches the dash. It is not a measured threshold, so no corpus could confirm or refute it.
